...
@section('title', 'Cập nhật phòng #' . $room->id . ' | FiveBees')
...
<form method="POST" action="{{ route('admin.rooms.update', [$room->id]) }}" enctype="multipart/form-data" id="editRoom">
    ...
    <div class="info-container d-flex flex-row">
        <div class="py-2 bg-white table-container" style="margin-right: 20px; border-radius: 10px; width: 70%; max-height: 650px;">
            <h3 class="mt-2 mx-2">Cập nhật phòng</h3>

            <div class="container">
                <div class="mb-3">
                    <label for="room_number" class="form-label">Số phòng:</label>
                    <input name="room_number" type="text" class="form-control" value="{{ old('room_number', $room->room_number) }}" required>
                    @error('room_number')
                ...
                <div class="mb-3">
                    ...
                    <input name="status" type="text" class="form-control" value="{{ old('status', $room->status) }}" required>
                    @error('status')
                    <p style="color: #dc3545">{{ $message }}</p>
                    @enderror
                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('.btn-remove-item').click(function(e) {
            ...
            $.ajax({
                method: "POST",
                url: "{{ route('admin.rooms.deleteMedia') }}",
                data: {
                    'photo_id': photo_id,
                },
